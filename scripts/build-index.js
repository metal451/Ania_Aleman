...
.lesson-card:hover .lesson-num { color: var(--gold); }
.lesson-body { flex: 1; }
.lesson-title {
  font-family: 'DM Serif Display', serif;
  font-size: 1.25rem;
...
.lesson-title em { font-style: italic; color: var(--gold); }
.lesson-desc {
  font-size: 0.88rem;
  color: var(--gray);
  font-weight: 300;
...
<div class="page">
  ...
  <footer>metal451.github.io/Ania_Aleman · 2026</footer>
</div>
</body>
</html>`;

// Generar HTML de lecciones
function generateLessonsHTML() {
  return lessons
    .map(lesson => `    <a class="lesson-card" href="${lesson.file}">
...
function buildIndex() {
  const lessonsHTML = generateLessonsHTML();
  const html = htmlTemplate.replace('${LESSONS_HTML}', lessonsHTML);
