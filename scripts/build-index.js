...
if (
  !nameMatch ||
  nameMatch[1].toLowerCase() !== target
) {
  continue;
}
...
return contentMatch
  ? decodeBasicEntities(contentMatch[1].trim())
  : '';
```
...
const customDescription = extractMeta(
html,
'lesson-description'
);

const standardDescription = extractMeta(
html,
'description'
);
...
const legacyLessons = LEGACY_LESSONS.filter(
lesson => {
const exists = fs.existsSync(
path.join(ROOT, lesson.file)
);

```
if (!exists) {
  console.warn(
    `Aviso: no se encontró ${lesson.file}; se omitirá del índice.`
  );
}
...
```
return a.titleText.localeCompare(
  b.titleText,
  'es',
  { sensitivity: 'base' }
);
...
<meta
name="description"
content="Lecciones interactivas de alemán para Ania."
...
<p class="subtitle">
  Microlecciones para corregir falencias específicas.
  Cada tarjeta abre una lección interactiva con audio
  y ejercicios.
</p>
```
...
```
  ...
  <div class="lessons">
    ${
      lessonCards ||
      '<div class="empty">Todavía no hay lecciones disponibles.</div>'
    }
  </div>

  <footer>
    <span>
      metal451.github.io/Ania_Aleman · 2026
    </span>

```
<span>
  Índice automático · ${generatedAt}
</span>
```
...
console.log(
`index.html generado con ${lessons.length} lección(es).`
);
];
...
const dynamicLessons = walkHtmlFiles(LESSONS_DIR)
  ...
  .sort((a, b) => {
    ...
    return a.titleText.localeCompare(b.titleText, 'es', {
      sensitivity: 'base'
    });
  });
